    ...
    <div :class="['layout', { collapse: isCollapse }]">
        ...
        <div class="cont">
            <div class="page-tab clearfix">
                <p v-for="(item, key) in pageTab" :class="{active: item.name === $route.name}" :key="key" v-if="item">
...
export default {
    ...
    computed: {
        pageTab(){
            return this.$store.state.pageTab
        }  
    },
    provide() {
        return {
    ...
    methods: {
        close(name){
            ...
            this.$router.replace(this.$store.state.pageTab[list[list.length - 1]].fullPath)
        }
    }
}
</script>
...
.layout {
    padding: 84px 0px 20px 240px;
    .trans;
    .cont{
        .page-tab{
            margin-bottom: 20px;
            p{
                float: left; height: 28px; line-height: 28px; padding: 0 8px 0 12px; 
                border-radius: 4px; margin-right: 10px; .bgf;
                a{color: #666666;}
                i{
